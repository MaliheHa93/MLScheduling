...
import MyMLScheduling.KNN_Trainting;

import MyKnapsack.WRPSAlgorithm;
import MyKnapsack.WRPSAlgorithm_SpareLp;
import MyKnapsack.MILPAlgorithm;
import MyKnapsack.MILPAlgorithmSpareTimeLp;
import MyKnapsack.MyDatacenter;

public class KNNScheduling extends DatacenterBroker{
	...
	public  Hashtable<String, Double> Scheduledworkflows;
	public Workflow w;	
	...
	public KNNScheduling(String name) throws Exception {
		super(name);
		...
		Scheduledworkflows= new Hashtable<>();
	...
	private void processTaskComplete(SimEvent ev) {
		...
		if(t.getStatus() == MyTask.STATUS_SUCCESS) {
			...
			int i = vmMipsIndex;
			int MAX_I = VMOffersGoogle.n1_standard_8;
			while (i <= MAX_I && !w.calculateSubDeadlineTasks(vmMips)) {
				vmMipsIndex = ++i;
				vmMips = VMOffersGoogle.getMips(vmMipsIndex);
				for (MyTask tw : w.waitingTasks) {
					tw.setEft(-1);
				}
			}
	 ...
	 * @throws Exception 
	 */
	private void StartSchedulingReadyTasks(SimEvent ev) throws Exception {	
	 ...
	 * @throws Exception 
	 */
	private void scheduleBot () throws Exception {
		submitWaitTasks();
	 ...
	 * @throws Exception 
	 */
	private void scheduleBotMultiTask(BagofTask bot) throws Exception {
		...
		if(bot.size() > 1 && idleVmList.size() == 0) {
			...
			while(list.hasNext() && !bot.isEmpty()) {
				...
				if(vm.getRemainingTimeSlot() == -1) {
					...
					vm.numberTask = (int)(duration/(sum_length * (VMOffersGoogle.getMips(VMOffersGoogle.n1_standard_1) / vm.getMips())));	
					if(vm.numberTask != 0) {
						for(int i = 0; i < vm.numberTask; i++) {
							MyTask t = bot.getNextReadyTaskFromBot();
							if(t!= null) {
	 ...
	 * @throws Exception 
	 **/
	private void provision(BagofTask bag) throws Exception {
		int[] NumTask = new int[initnewVmList.size()];
		double[] cost = new double[initnewVmList.size()];
		double[] maxProcessTime = new double[initnewVmList.size()] ;
		int[] maxInstanceIndex = new int[initnewVmList.size()];
		...
		KNNProvision knnp = new KNNProvision();
		MyTask largestTask =null;
		
		for(MyTask t:bag) {
			if(max < t.getTaskruntime()) {
				max = t.getTaskruntime();
				largestTask = t;
			}
		}
		
		for(MyTask t:bag) {
			mean += t.getTaskruntime();
			mean/= bag.size();	
		}
		...
		for(MyVm vmoffer :initnewVmList) {
			...
			NumTask[id] = (int)(duration / (mean * (VMOffersGoogle.getMips(VMOffersGoogle.n1_standard_1) / vmoffer.getMips())));
			cost[id] =  VMOffersGoogle.getCost(vmoffer.getMips(), vmoffer.getRam(), vmoffer.getBw());
			maxProcessTime[id] = mean * (VMOffersGoogle.getMips(VMOffersGoogle.n1_standard_1) / vmoffer.getMips());
			
			if(NumTask[id]!= 0) {
				maxInstanceIndex[id] = (int) Math.ceil((double)bag.size() / NumTask[id]);
			}else {
				maxInstanceIndex[id] = 0;
			}
		}		
		/*calculate number of resources for scheduling bag by DP*/
		knnp.Train(w.tasklist.size(), initnewVmList, Scheduledworkflows, cost, MILPAlgorithmSpareTimeLp.recordBOTMILP, MILPAlgorithmSpareTimeLp.recordBOPMILP, WRPSAlgorithm.recordBOTWRPS, WRPSAlgorithm.recordBOPWRPS);
		vmProvidingResult result = knnp.ProvisionBoT(bag,bag.size(),maxInstanceIndex ,duration, NumTask, cost, maxProcessTime);
	 ...
	 * @throws Exception 
	 */
	private void scheduleBop() throws Exception {
		scheduleWaitPipe();	
	 ...
	 * @throws Exception 
	 */
	private void scheduleBagofPipe(BagofPipline bop) throws Exception {
		boolean applicaple = false;
		double sumRunTimeTaskPie = 0;
		double maxLengthPipe = 0;
		double meanLengthPipe = 0;
...
//		}
		...
		for(Pipeline p :bop) {			
			meanLengthPipe += p.getSumPipeRunTime();
			meanLengthPipe/= bop.size();
		}
		if(bop.size() > 1 && idleVmList.size() == 0) {
			...
			while(vmList.hasNext() && !bop.isEmpty()) {
				...
				if(vm.getRemainingTimeSlot() == -1) {
					...
					vm.numberPipeline =  (int) (duration/ (meanLengthPipe * (VMOffersGoogle.getMips(VMOffersGoogle.n1_standard_1) / vm.getMips())));
...
//								}
	 ...
	 * @throws Exception 
	 */
	private void provisionBagofPiepline(BagofPipline bop) throws Exception {
		...
		Hashtable<Integer, MyVm> getvm = new Hashtable<Integer, MyVm>();
		KNNProvision knn = new KNNProvision();
		double deadlineBop = bop.getDeadlineBop();
		double sumRunTimeTaskPie = 0;
		double medRunTime = 0.0;
		double maxLengthPipe = 0;
...
//		}
		for(Pipeline p :bop) {
			...
			p.setSumPipeRunTime(sumRunTimeTaskPie);	
			sumRunTimeTaskPie= 0.0;
		}
		for(Pipeline p :bop) {
			medRunTime += p.getSumPipeRunTime()	;
			medRunTime /= bop.size();
		}
		...
		for(MyVm vm : initnewVmList) {
			id = vm.getId();
			NumPipeline[id] = (int)(duration/ (medRunTime* (VMOffersGoogle.getMips(VMOffersGoogle.n1_standard_1) / vm.getMips())));			
			cost[id] = VMOffersGoogle.getCost(vm.getMips() ,vm.getRam(), vm.getBw());
			getvm.put(vm.getId(), vm);
			maxProcessTime[id] = medRunTime * (VMOffersGoogle.getMips(VMOffersGoogle.n1_standard_1) / vm.getMips());
			if(NumPipeline[id]!= 0) {
				maxInstanceIndex[id]  = (int) Math.ceil((double) bop.size() / NumPipeline[id]);
			}else {
				maxInstanceIndex[id] = 0;
			}
		}
		knn.Train(w.tasklist.size(), initnewVmList, Scheduledworkflows, cost, MILPAlgorithmSpareTimeLp.recordBOTMILP, MILPAlgorithmSpareTimeLp.recordBOPMILP, WRPSAlgorithm.recordBOTWRPS, WRPSAlgorithm.recordBOPWRPS);
		vmProvidingResult result = knn.ProvisionBoP(bop, bop.size(),maxInstanceIndex ,duration,NumPipeline, cost, maxProcessTime);
		for(ResourceProvisioning rp : result.resources) {
			id = rp.vmid;
			MyVm vms = getvm.get(id);
			int pe = vms.getNumberOfPes();
...
//			throw new RuntimeException();
		...
		MyVm newvm = createNewVm(pe);
		newvm.setCostPerMinute(VMOffersGoogle.getCost(pe));
		newvm.setnumberPipelineAssign(1);
		sendNow(dataCenterId, CloudSimTags.REQUEST_VM , newvm);	
	}
			...
			for(MyTask t: p) {
				if(idleVm.submitTaskflag == true) {
					...
					if(t.getStatus() == MyTask.STATUS_READY) {
						...
						for(Integer key : w.getNumberTaskinLevel().keySet()) {
							ArrayList<MyTask> itr = w.getNumberTaskinLevel().get(key);
							if(itr.contains(t)) {
								itr.remove(t);
//								if(itr.isEmpty()) {
//									w.getNumberTaskinLevel().remove(key);
//								}
							}
						}
...
//					}
	...
	private void initialBroker(SimEvent ev) throws Exception {
		...
		Log.printLine("**************************************************************");
		Log.printLine("**************************************************************");
		Log.printLine("------------.****starting KNN scheduling****.------------");
		Log.printLine("**************************************************************");
		Log.printLine("**************************************************************");
		...
		/*set DaxPath*/
		String daxpath = "D:/Master Material/dataDrivenScheduling/examples/dax/Inspiral_10000.xml";
		...
		vmMips = VMOffersGoogle.getMips(VMOffersGoogle.n1_standard_1);
		vmMipsIndex = 1;
		...
		int i = VMOffersGoogle.n1_standard_1;
		int MAX_I = VMOffersGoogle.n1_standard_8;
...
//		WRPSCost
		Scheduledworkflows.put("WRPS_Cost",83.65430676914806);
		Scheduledworkflows.put("WRPS_Makespan",176.72741299998486);

		//		MHPS Cost
		Scheduledworkflows.put("MHPS_Cost",61.547797087499625);
		Scheduledworkflows.put("MHPS_Makespan",424.5695529997595);
...
//		TrainData();
	...
	/**
	 * Now we must train our algorithm with list of our tasks, processing power and objective of our problem
	 * call KNN_Training class method
	 * @throws Exception 
	 */
	
//	private void TrainData() throws Exception {
//		
//		List<MyVm>vmlist= initnewVmList;
//		ArrayList<Double> costlist = new ArrayList<Double>();
//		
//
//		
//		for(MyVm vm: vmlist) {
//			costlist.add(VMOffersGoogle.getCost(vm.getNumberOfPes()));
//		}
        
//		KNN_Trainting knt= new KNN_Trainting();		
