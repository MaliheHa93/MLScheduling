...
import MyMLScheduling.Pipeline;
import MyMLScheduling.MyTask;
...
public class Workflow {
	...
	public Workflow(ArrayList<MyTask> tasklist) {
		...
		setNumLevelTask(new Hashtable<Integer, Integer>());
	 ...
	 * @return
	 */
	private Pipeline findPipline(MyTask t, Pipeline pipeline2, List<MyTask> ischeck) {
		int cNum;
		ischeck.add(t);
		cNum = t.getChildList().size();
		if (cNum > 1 || cNum == 0 || t.getChildList().get(0).getParentList().size() > 1) {
			if (!pipeline2.isEmpty())
				t.putInPipeline(pipeline2);
			return pipeline2;
		}
		t.putInPipeline(pipeline2);
		return findPipline(t.getChildList().get(0), pipeline2, ischeck);
	}
