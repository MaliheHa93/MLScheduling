...
public class WRPSAlgorithm_SpareLp extends DatacenterBroker{
	...
	public WRPSAlgorithm_SpareLp(String name) throws Exception {		
		super(name);
...
//					double needTime = vm.numberTask * largestTask.getEstimatedRunTime(vm.getMips());
//					if(needTime + CloudSim.clock() < vm.getEndTimeSlot()) {
						if(vm.numberTask != 0) {
							for(int i = 0; i < vm.numberTask; i++) {
								MyTask t = bot.getNextReadyTaskFromBot();
								if(t!= null) {
...
//					}
//					else {
//						double d1 = vm.getEndTimeSlot() - CloudSim.clock();
//						vm.numberTask = (int) (d1 / largestTask.getEstimatedRunTime(vm.getMips()));
//						if(vm.numberTask != 0) {
//							for(int i = 0; i < vm.numberTask; i++) {
//								MyTask t = bot.getNextReadyTaskFromBot();
//								if(t!= null) {
...
//					double needTime = vm.numberPipeline * largestp.getEstimatedRunTime(vm.getMips());
//					if(needTime + CloudSim.clock() < vm.getEndTimeSlot()) {
...
//					else {
//						double d1 = vm.getEndTimeSlot() - CloudSim.clock();
//						vm.numberPipeline = (int) (d1 / largestp.getEstimatedRunTime(vm.getMips()));
//						if(vm.numberPipeline != 0) {
//							for(int i = 0; i < vm.numberPipeline; i++) {
//								Pipeline p = bop.getNextReadyPipelineFromBop();	
//								if(p!= null) {
...
//		requestedVmList.add(newvm);
	...
	private void initialBroker(SimEvent ev) throws Exception {
		...
		Log.printLine("**************************************************************");
		Log.printLine("**************************************************************");
		Log.printLine("------------.****starting Knapsack+LPspare scheduling****.------------");
		Log.printLine("**************************************************************");
		Log.printLine("**************************************************************");
		//get data center id
