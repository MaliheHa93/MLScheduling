...
public class WRPSAlgorithm extends DatacenterBroker{
	...
	public static Hashtable<BagofTask,KnapsackResult>recordBOTWRPS;
	public static Hashtable<BagofPipline,KnapsackResult>recordBOPWRPS;
	
	public  ArrayList<Pipeline> pipeline;
	public ArrayList<Workflow> scheduledWorkflow;
	...
	public WRPSAlgorithm(String name) throws Exception {		
		super(name);
		...
		scheduledWorkflow = new ArrayList<Workflow>();
		allVmMap = new HashMap<>();
		assignPipeline = new Hashtable<>();
		
		recordBOTWRPS= new Hashtable<BagofTask, KnapsackResult>();
		recordBOPWRPS= new Hashtable<BagofPipline, KnapsackResult>();
	...
	private void StartSchedulingReadyTasks(SimEvent ev) {	
		...
		for(int i = 0; i < idleVmList.size();i++) {
			MyVm vm  =idleVmList.get(i);
			if(vm.Time2BillingPeriod() < 5) {
				freeVM(vm);
				i--;
			}
		}
	...
	private void provision(BagofTask bag) {
		...
		for(MyVm vmoffer :initnewVmList) {
			...
			maxProcessTime[id] = largestTask.getEstimatedRunTime(vmoffer.getMips()) + largestTask.getTransferTime();
			if(NumTask[id]!= 0) {
		...
		KnapsackResult result = ukp.DP(bag.size(),maxInstanceIndex ,duration, NumTask, cost,maxProcessTime, 1e-05);
		recordBOTWRPS.put(bag, result);
...
//		System.err.println("TotalCost " + totalCost);
	...
	private void provisionSingleTaskVm(BagofTask bot) {
		...
		r.resources.add(rp);
		recordBOTWRPS.put(bot, r.getResources(rp));
...
//								}
	...
	private void provisionBagofPiepline(BagofPipline bop) {
		...
		for(MyVm vm : initnewVmList) {
			...
			maxProcessTime[id] = largestp.getEstimatedRunTime(vm.getMips()) + largestp.getTransferTime();
			if(NumPipeline[id]!=0) {
		...
		KnapsackResult result = ukp.DP(bop.size() ,maxInstanceIndex ,duration, NumPipeline , cost, maxProcessTime, 0);
		recordBOPWRPS.put(bop, result);
	...
	private void provisionSinglePipeline(BagofPipline bop) {
		...
		ResourceProvisioning rp = new ResourceProvisioning(minIndex, 1, 1, 1);
		r.resources.add(rp);
		recordBOPWRPS.put(bop, r.getResources(rp));
	...
	private void initialBroker(SimEvent ev) throws Exception {
		...
		Log.printLine("**************************************************************");
		Log.printLine("**************************************************************");
		Log.printLine("------------.****starting Knapsack scheduling****.------------");
		Log.printLine("**************************************************************");
		Log.printLine("**************************************************************");
		//get data center id
	...
	public void shutdownEntity() {
		...
		System.out.println("Slot " + MyDatacenter.slot);
		System.out.println("wasteSlot " + MyDatacenter.wasteTimeslot);
		System.out.println("totalVm " + MyDatacenter.totalNum);
		System.out.println("Makespan : " + (MakeSpan - startTime) + " seconds");
		System.out.println("SumRunTime " + MyVm.sumRunTime);
	}
}
