...
public class MILPAlgorithmSpareTimeLp extends DatacenterBroker{
	...
	public static Hashtable<BagofTask,KnapsackResult>recordBOTMILP;
	public static Hashtable<BagofPipline,KnapsackResult>recordBOPMILP;
	...
	public MILPAlgorithmSpareTimeLp(String name) throws Exception {		
		super(name);
		...
		recordBOTMILP= new Hashtable<BagofTask, KnapsackResult>();
		recordBOPMILP= new Hashtable<BagofPipline, KnapsackResult>();
	...
	private void processTaskComplete(SimEvent ev) {
		...
		if(t.getStatus() == MyTask.STATUS_SUCCESS) {
			...
			w.assignSubDeadlineTaskswithLpSpare(vmMips);
...
//		}
	...
	private void provision(BagofTask bag) {
		...
		provisionMilp milp = new provisionMilp();
		MyTask largestTask =null;
		...
		KnapsackResult result = milp.solveMilp(bag.size(),maxInstanceIndex ,duration, NumTask, cost, maxProcessTime, 1e-05);
		recordBOTMILP.put(bag, result);
...
//		sum_length /= bot.size();
	 ...
	 * @param t
	 */
	private void provisionSingleTaskVm(BagofTask bot) {
		int[] NumTask = new int[initnewVmList.size()];
		double[] cost = new double[initnewVmList.size()];
		int id = -1;
		
		double duration = bot.get(0).getsubDeadline()  - CloudSim.clock();

		for(MyVm vmoffer :initnewVmList) {
			id = vmoffer.getId();
			NumTask[id] = (int)(duration / bot.get(0).getEstimatedRunTime(vmoffer.getMips()));
			cost[id] = VMOffersGoogle.getCost(vmoffer.getMips(), vmoffer.getRam(), vmoffer.getBw());
		}
		...
		newvm.setAssingedTask(bot.get(0));
		
		KnapsackResult r = new KnapsackResult();
		ResourceProvisioning rp = new ResourceProvisioning(minIndex, 1, 1, 1);
		r.resources.add(rp);
		recordBOTMILP.put(bot, r.getResources(rp));
...
//			}				
		...
		KnapsackResult result = milp.solveMilp(bop.size(),maxInstanceIndex ,duration,NumPipeline, cost, maxProcessTime, 0);
		for(ResourceProvisioning rp : result.resources) {
			id = rp.vmid;
			MyVm vms = getvm.get(id);
			int pe = vms.getNumberOfPes();
...
//			}
		...
		if(bop.size() == 1 && idleVmList.size() > 0) {
			...
			while(vmList.hasNext() && !bop.isEmpty()) {
				...
				if(vm.getRemainingTimeSlot() == -1) {
					...
					double duration = bop.getDeadlineBop() - CloudSim.clock();
					vm.numberPipeline = (int)(duration/ bop.get(0).getEstimatedRunTime(vm.getMips()));
					if(vm.numberPipeline != 0) {
						Pipeline p = bop.getNextReadyPipelineFromBop();
						bop.RemoveFromBop(p);
						boolean f = submitBopVM(p, vm);
	 ...
	 * @param deadlineBop
	 */
	private void provisionSinglePipeline(BagofPipline bop) {
		int[] NumTask = new int[initnewVmList.size()];
		double[] cost = new double[initnewVmList.size()];
		int id =-1;
		double sumTaskRunTimePipe = 0;
		double deadlinep = bop.get(0).getDeadline();
		for(MyTask t: bop.get(0)) {
			sumTaskRunTimePipe += t.getTaskruntime();			
		}	
		bop.get(0).setSumPipeRunTime(sumTaskRunTimePipe);
		double duration = deadlinep - CloudSim.clock();

		for(MyVm vm: initnewVmList) {
			id = vm.getId();
			NumTask[id] = (int)(duration /(bop.get(0).getEstimatedRunTime(vm.getMips())));
			cost[id] = (VMOffersGoogle.getCost(vm.getMips(), vm.getRam(), vm.getBw()));
		} 
...
//			throw new RuntimeException();
		...
		ResourceProvisioning rp = new ResourceProvisioning(minIndex, 1, 1, 1);
		r.resources.add(rp);
		recordBOPMILP.put(bop, r.getResources(rp));
...
//					}
	...
	private void initialBroker(SimEvent ev) throws Exception {
		...
		Log.printLine("**************************************************************");
		Log.printLine("**************************************************************");
		Log.printLine("------------.****starting MILP scheduling****.------------");
		Log.printLine("**************************************************************");
		Log.printLine("**************************************************************");
		//get data center id
		...
		vmMips = VMOffersGoogle.getMips(VMOffersGoogle.n1_standard_8);
		vmMipsIndex = 1;
		...
		/*calculate deadline on fastest vm*/
		w.assignSubDeadlineTaskswithLpSpare(vmMips);
